...
async function initializeDatabase() {
  try {
    ...
    // Create repayment_logs table if it doesn't exist
    await pool.query(`
    ...
    await pool.query(`
      DROP TRIGGER IF EXISTS after_repayment_insert;
    `);
    ...
    console.log('Database connected successfully, tables and triggers initialized');
  } catch (error) {
    console.error('Failed to connect to database or initialize triggers:', error.message);
    process.exit(1);
  }
}
...
// loan Types
app.get('/api/loan_types', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT id, type AS name FROM loan_types');
    if (rows.length === 0) {
      return res.status(404).json({ error: 'No loan types found' });
    }
    res.json(rows);
  } catch (error) {
    console.error('Error fetching loan types:', error);
    res.status(500).json({ error: 'Failed to fetch loan types', details: error.message });
  }
});
...
app.post('/api/reservations', verifyToken, async (req, res) => {
  ...
  if (!customer_id || !loan_type_id || !start_date || !end_date || !amount || !terms_agreed) {
    return res.status(400).json({ error: 'Missing required fields' });
  }
  ...
  try {
    const [result] = await pool.query(
      `INSERT INTO reservations (
        customer_id, loan_type_id, start_date, end_date, purpose, amount, status,
...
app.get('/api/admin/dashboard', verifyToken, isAdmin, async (req, res) => {
  try {
    const [reservations] = await pool.query(`
      ...
      FROM reservations r
      JOIN loan_types rt ON r.loan_type_id = rt.id
...
app.post('/api/repayments', verifyToken, async (req, res) => {
  ...
  console.log('Received repayment request:', { customer_id, reservation_id, amount });
  ...
  try {
    ...
    const [result] = await connection.query(
      'INSERT INTO repayments (loan_id, amount, due_date, status) VALUES (?, ?, CURDATE(), ?)',
      [reservation_id, paymentAmount, 'paid']
    );
